...
//*******************************************************/
// Zano note:
// This file makes an override in the getCypressExecutable
// to make use of the cypress-ntlm executable so that tagging will still work.
// This file gets copied back to the cypress cucumber node modules folders
// to replace the cypress-tags.js file
//******************************************************/
...
function getCypressExecutable() {
	const command = getOsSpecificExecutable(`${__dirname}/../.bin/cypress-ntlm`);
	// fallback to the globally installed cypress instead
	return fs.existsSync(command)
		? command
		: getOsSpecificExecutable("cypress-ntlm");
}
